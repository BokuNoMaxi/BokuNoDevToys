...
<div class="max-w-4xl mx-auto">
	<div class="mb-10">
		...
		<p class="mt-2 text-slate-400">{$t('home').subtitle}</p>
	</div>
		...
		{#each categories as category}
			...
			<div>
				<div class="flex items-center gap-2 mb-4">
					...
					<h2 class="text-sm font-semibold text-slate-500 uppercase tracking-wider">{cats[category.key]}</h2>
				</div>
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
					{#each category.tools as tool}
						...
						>
							...
							<p class="mt-1.5 text-sm text-slate-500 group-hover:text-slate-400 transition-colors">{meta?.description ?? ''}</p>
						</a>
					{/each}
				</div>
			</div>
	...
	<div class="mt-12 pt-8 border-t border-slate-800 text-center">
		<p class="text-xs text-slate-600">{$t('home').footer}</p>
	</div>
</div>
