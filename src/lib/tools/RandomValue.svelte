...
<div class="space-y-4">
	<div class="bg-slate-800 rounded-xl p-6 space-y-4">
		...
		<div>
			...
			></textarea>
			<p class="mt-1.5 text-xs text-slate-300">{values().length} {$t('randomValue').entries}</p>
		</div>
		<button onclick={pick} disabled={values().length === 0}
	...
	{#if history.length > 1}
		<div class="bg-slate-800 rounded-xl p-6">
			<h2 class="text-sm font-semibold text-slate-300 uppercase tracking-wider mb-3">{$t('randomValue').history}</h2>
