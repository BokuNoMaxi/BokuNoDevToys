...
<div class="space-y-6">
	<div class="bg-slate-800 rounded-xl p-6 space-y-5">
		<h2 class="text-sm font-semibold text-slate-300 uppercase tracking-wider">{$t('timezone').title}</h2>

		<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<div>
				<label class="block text-xs text-slate-300 mb-1.5" for="tz-date">{$t('timezone').date}</label>
			...
			<div>
				<label class="block text-xs text-slate-300 mb-1.5" for="tz-time">{$t('timezone').time}</label>
		...
		<div class="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-3 items-end">
			<fieldset>
				<legend class="text-xs text-slate-300 mb-2 uppercase font-medium">{$t('timezone').from}</legend>
			...
			<fieldset>
				<legend class="text-xs text-slate-300 mb-2 uppercase font-medium">{$t('timezone').to}</legend>
				<div class="flex gap-2">
		...
		<button onclick={convert} class="w-full px-5 py-2.5 bg-violet-700 hover:bg-violet-800 text-white rounded-lg font-medium transition-colors">
			{$t('timezone').convert}
		</button>
		...
		{#if result}
			<div class="flex items-center gap-3 bg-slate-900 rounded-lg px-4 py-3">
				...
				<button onclick={copy} class="text-xs text-slate-300 hover:text-slate-100 transition-colors">{$t('timezone').copy}</button>
			</div>
		{/if}
	</div>
</div>
