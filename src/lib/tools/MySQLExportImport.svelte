...
<div class="space-y-6">
	...
	<!-- Connection Fields -->
	<div class="bg-slate-800 rounded-xl p-6">
		<h2 class="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-5">{$t('mysql').connectionParams}</h2>
		<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<div>
				<label for="db-user" class="block text-xs text-slate-500 mb-1.5">{$t('mysql').user} <span class="text-red-500">*</span></label>
			...
			<div>
				<label for="db-pass" class="block text-xs text-slate-500 mb-1.5">{$t('mysql').password}</label>
				<input id="db-pass" type="password" bind:value={password} placeholder="••••••••"
					class="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500 font-mono text-sm" />
			</div>
			<div>
				<label for="db-host" class="block text-xs text-slate-500 mb-1.5">{$t('mysql').host}</label>
				<input id="db-host" type="text" bind:value={host} placeholder="localhost"
					class="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500 font-mono text-sm" />
			</div>
			<div>
				<label for="db-name" class="block text-xs text-slate-500 mb-1.5">{$t('mysql').database} <span class="text-red-500">*</span></label>
				<input id="db-name" type="text" bind:value={database} placeholder="my_database"
					class="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500 font-mono text-sm" />
			</div>
			<div class="sm:col-span-2">
				<label for="db-file" class="block text-xs text-slate-500 mb-1.5">{$t('mysql').filename}</label>
		...
		<div class="space-y-4">
			...
			<div class="bg-slate-800 rounded-xl p-6">
				<div class="flex items-center justify-between mb-3">
					<div>
						<h2 class="text-sm font-semibold text-slate-300">Export <span class="ml-2 text-xs font-normal text-slate-500">{$t('mysql').exportSubtitle}</span></h2>
					</div>
					<button
			...
			<div class="bg-slate-800 rounded-xl p-6">
				<div class="flex items-center justify-between mb-3">
					<div>
						<h2 class="text-sm font-semibold text-slate-300">Import <span class="ml-2 text-xs font-normal text-slate-500">{$t('mysql').importSubtitle}</span></h2>
					</div>
					<button
	...
	{:else}
		<div class="bg-slate-800/50 border border-dashed border-slate-700 rounded-xl p-8 text-center">
			<p class="text-slate-600 text-sm">{$t('mysql').fillRequired}</p>
		</div>
	{/if}
</div>
