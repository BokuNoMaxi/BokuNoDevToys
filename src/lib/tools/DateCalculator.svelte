...
<div class="space-y-8">
	<div class="bg-slate-800 rounded-xl p-6 space-y-5">
		<h2 class="text-sm font-semibold text-slate-300 uppercase tracking-wider">{$t('dateCalc').calcTitle}</h2>

		<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
			<div>
				<label class="block text-xs text-slate-300 mb-1.5" for="dc-date">{$t('dateCalc').startDate}</label>
			...
			<div>
				<label class="block text-xs text-slate-300 mb-1.5" for="dc-time">{$t('dateCalc').startTime}</label>
		...
		<div class="flex gap-3" role="group" aria-label="{$t('dateCalc').add} / {$t('dateCalc').subtract}">
			<button onclick={() => operation = 'add'}
				...
				class="flex-1 py-2 rounded-lg font-medium transition-colors {operation === 'add' ? 'bg-emerald-800 text-white' : 'bg-slate-700 text-slate-200 hover:text-white'}">
				{$t('dateCalc').add}
			</button>
			<button onclick={() => operation = 'sub'}
				aria-pressed={operation === 'sub'}
				class="flex-1 py-2 rounded-lg font-medium transition-colors {operation === 'sub' ? 'bg-rose-800 text-white' : 'bg-slate-700 text-slate-200 hover:text-white'}">
				{$t('dateCalc').subtract}
			</button>
		</div>
		...
		<div class="grid grid-cols-3 sm:grid-cols-6 gap-3">
			{#each [
				...
				<div>
					<label class="block text-xs text-slate-300 mb-1.5" for="dc-{field.labelKey}">{$t('dateCalc')[field.labelKey]}</label>
		...
		<button onclick={calculate} class="w-full px-5 py-2.5 bg-violet-700 hover:bg-violet-800 text-white rounded-lg font-medium transition-colors">
			{$t('dateCalc').calculate}
		</button>

		{#if error}<p class="text-red-300 text-sm" role="alert">{error}</p>{/if}
		{#if result}
			<div class="flex items-center gap-3 bg-slate-900 rounded-lg px-4 py-3">
				...
				<button onclick={() => copy(result)} class="text-xs text-slate-300 hover:text-slate-100 transition-colors">{$t('dateCalc').copy}</button>
			</div>
		{/if}
	</div>

	<div class="bg-slate-800 rounded-xl p-6 space-y-5">
		<h2 class="text-sm font-semibold text-slate-300 uppercase tracking-wider">{$t('dateCalc').diffTitle}</h2>

		<div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
			<fieldset class="space-y-3">
				<legend class="text-xs text-slate-300 font-medium uppercase">{$t('dateCalc').dateA}</legend>
			...
			<fieldset class="space-y-3">
				<legend class="text-xs text-slate-300 font-medium uppercase">{$t('dateCalc').dateB}</legend>
		...
		<button onclick={calculateDiff} class="w-full px-5 py-2.5 bg-violet-700 hover:bg-violet-800 text-white rounded-lg font-medium transition-colors">
			{$t('dateCalc').calcDiff}
		</button>
